    ...
    for (let i = 0; i < totalToProcess; i++) {
      ...
      if (!matchedAnime) {
        ...
        const newAnime: Anime = {
          ...
          startDate: item.aired || (item.year ? String(item.year) : ''),
          endDate: '',
          season: item.season ? `${item.season.charAt(0).toUpperCase() + item.season.slice(1)} ${item.year || ''}`.trim() : (item.year ? String(item.year) : ''),
